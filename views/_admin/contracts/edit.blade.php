...
<section class="content">
    <div class="row">
        <div class="col-xs-12">
			...
			<form enctype="multipart/form-data" data-edit="true" class="form-horizontal form-label-left ajax_form" method="post" action="{{route('contracts.update', $contract->id)}}">
			{{ csrf_field() }}
			{{method_field('PUT') }}
			<h3 style="text-align:center">{{__('admin.edit_token',['token'=>$contract->name])}}</h3>
			<hr>
				...
				<div class="item form-group">
				  ...
				  <div class="col-md-6 col-sm-6 col-xs-12">
                  	<textarea rows="6" class="form-control col-md-7 col-xs-12" id="abi" name="abi" placeholder="{{trans('admin.enter_abi')}}">{{$contract->abi or ""}}
					</textarea>
				  </div>
                 </div>
				 ...
				 <div class="item form-group">
				  ...
				  <div class="col-md-6 col-sm-6 col-xs-12">
                  	<textarea rows="6" class="form-control col-md-7 col-xs-12" id="mainsale_abi" name="mainsale_abi" placeholder="Mainsale(ICO ) ABi id ICO contract is different from the token contract">{{$contract->mainsale_abi or ""}}
					</textarea>
				  </div>
                 </div>
